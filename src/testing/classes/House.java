 ...
 * signature line with a comma. Also, the first first generic "T" is a 
 * restricted generic. this means you can use any data type as long as it fits
 * the restrictions. This means you can use any Person object, or any of the 
 * children of the Person class. 
 *
 * @author Mr. Wachs
 * @since Nov 12, 2024, 11:34:30 a.m.
 */
public class House <T extends Person, U>
{
    ...
    public <V extends Food> void party(V[] items) {
        System.out.println("Home owner: " + homeOwner.name);
        System.out.println("\tAnd the contents: " + contents.toString());
        // Enhanced for loop ("For every item in items")
        for (V item : items) {
            homeOwner.consume(item);    // Calling the method on abstract class
        }
    }
